...
$(function () {
    document.onkeydown = function (e) {
        ...
        if (ev.keyCode == 13) { // 如（ev.ctrlKey && ev.keyCode==13）为ctrl+Center 触发
            //alert($('#PatName-input').val());
            var PatientName = $('#PatName-input').val()
            $.ajax({
                url: `http://localhost:40415/Service.asmx/GetSameNamePat`,
                data: {
                    "name": PatientName
                },
                dataType: "xml",
                ...
                success: function (data) {
                    console.log('get sameNamePat successfully!')
                    console.log(data)
                    var sameNamePats = data.getElementsByTagName('QuerySameNamePat')
                    clearPatientTable()
                    if (sameNamePats) {
                        for (var i = 0; i < sameNamePats.length; i++) {
                            var patSEQ = sameNamePats[i].getElementsByTagName('PatSEQ')[0] ? sameNamePats[i].getElementsByTagName('PatSEQ')[0].innerHTML : ''
                            //var patID=sameNamePats[i].getElementsByTagName('PatID')[0] ? sameNamePats[i].getElementsByTagName('PatID')[0].innerHTML : ''
                            var patNum = i + 1
                            var patName = sameNamePats[i].getElementsByTagName('PatName')[0] ? sameNamePats[i].getElementsByTagName('PatName')[0].innerHTML : ''
                            var patBirthday = sameNamePats[i].getElementsByTagName('PatBirthday')[0] ? sameNamePats[i].getElementsByTagName('PatBirthday')[0].innerHTML : ''
                            addSamePatientItem(patSEQ, patNum, patName, patBirthday)
                        }
                    }
                },
                error: function (data) {
...
function addSamePatientItem(PatSEQ, PatNum, PatName, PatBirthday) {
    let List = $(`#samePat-list`)
    let newItem = $(`<tr id=\"pat-list-${PatSEQ}\"></tr>`)
    let oneItem = $(`<td>${PatNum}</td>`)
    let twoItem = $(`<td>${PatName}</td>`)
    let threeItem = $(`<td>${PatBirthday}</td>`)
    newItem.click(function () {
        selectedPat = $(this).attr("id").split('pat-list-')[1]
        getSelectedPatData(selectedPat)
    })
...
function getSelectedPatData(Seq) {
    ...
    $.ajax({
        url: `http://localhost:40415/Service.asmx/GetOnlyDataFromDBByRecordSEQ`,
        data: {
            "PatSEQ": Seq
        },
        dataType: "xml",
        method: 'POST',
        success: function (data) {
            console.log('get selected PatData successfully!')
            console.log(data)
