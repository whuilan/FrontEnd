  ...
  function addPatientItem(PatSeq, ID, Name, Sex) {
  ...
  function getSinglePatData(Seq,VisitDate) {
    ...
    $.ajax({
      ...
      success: function (data) {
        console.log(data)
        setDisHistoryTable(data,VisitDate)
      },
      error: function (data) {
  ...
  function setDisHistoryTable(data,VisitDate) {
    ...
    if (diseases) {
      ...
      for (var i = 0; i < diseases.length; i++) {
        ...
        addDisHistoryItem(rec, time, syn, deg, sco, lost)

      }
    }
  }

  function addDisHistoryItem(Rec, Time, Syn, Deg, Sco, Lost) {
        ...
        if (pats) {
          ...
          for (var i = 0; i < pats.length; i++) {
            ...
            if (!patDict[patSeq]) {
              ...
              addPatientItem(patSeq, ID, name, sex)
            } else {
              patDict[patSeq].push(seq) // tang: [1, 2. 3]
              patTimeDict[patSeq].push(time)
            }
          }
          console.log(patDict) // { tang: [1,2,4,5], 123: [1].... }
          console.log(patTimeDict)
        }
        // PatList.map (item => {
